...
## R uses {} to know when portions of code end and begin
```
...
test_temp <- c(16.550540, 26.942005,  32.591088, 8.580116, 28.003873, 28.542391, 20.076820, 21.173577, 21.048836, 18.769735)
...
# Margin = 1 is for rows, 2 is for columns
...
```{r}
test_temp_mat <- matrix(c(16.550540, 26.942005,  32.591088, 8.580116, 28.003873, 28.542391, 20.076820, 21.173577, 21.048836, 18.769735), nrow = 2)
...
1.  Do 5 CodeWars problems in R. Share your solutions here:
    ...
    ```{r}
    # 1) Create a function that indicates if an integer is even or odd
    ...
    # 2) Determine how many breaks a chocolate bar needs to produce 1x1 squares,
    #    Return 0 if invalid/no answer
    # Url: https://www.codewars.com/kata/534ea96ebb17181947000ada/train/r
    ...
    print(break_chocolate(5, 2))

    # 3) Return the distance between pillars on the side of the road.
    # url; https://www.codewars.com/kata/5bb0c58f484fcd170700063d/train/r

    pillars = function(num_of_pillars, distance, width) {
      pill_distance <- if(num_of_pillars < 2) {
        return(0)
      ...
      return(pill_distance)
      
    }

    print(pillars(5, 10, 15))

    # 4) Return the total sum of the interior angles of an n-sided polygon.
    # url: https://www.codewars.com/kata/5a03b3f6a1c9040084001765/train/r
    ...
    # 5) Return the next happy year (year with distint digits only)
    # url: https://www.codewars.com/kata/5ae7e3f068e6445bc8000046/train/r

    next_happy_year <- function(year) {
      nextYear <- as.integer(year) + 1
      isHappyYear <- length(unique(strsplit(as.character(nextYear), "")[[1]])) == nchar(as.character(year))
      if (!isHappyYear) {
        return(next_happy_year(nextYear))
      }
      return(nextYear)
    }

    print(next_happy_year(1988))
    ```
